var allFrequencies = [
    8.1757989156,
    8.661957218,
    9.1770239974,
    9.7227182413,
    10.3008611535,
    10.9133822323,
    11.5623257097,
    12.2498573744,
    12.9782717994,
    13.75,
    14.5676175474,
    15.4338531643,
    16.351597831287414,
    17.323914436054505,
    18.354047994837977,
    19.445436482630058,
    20.601722307054366,
    21.826764464562746,
    23.12465141947715,
    24.499714748859326,
    25.956543598746574,
    27.5,
    29.13523509488062,
    30.86770632850775,
    32.70319566257483,
    34.64782887210901,
    36.70809598967594,
    38.890872965260115,
    41.20344461410875,
    43.653528929125486,
    46.2493028389543,
    48.999429497718666,
    51.91308719749314,
    55,
    58.27047018976124,
    61.7354126570155,
    65.40639132514966,
    69.29565774421802,
    73.41619197935188,
    77.78174593052023,
    82.4068892282175,
    87.30705785825097,
    92.4986056779086,
    97.99885899543733,
    103.82617439498628,
    110,
    116.54094037952248,
    123.47082531403103,
    130.8127826502993,
    138.59131548843604,
    146.8323839587038,
    155.56349186104046,
    164.81377845643496,
    174.61411571650194,
    184.9972113558172,
    195.99771799087463,
    207.65234878997256,
    220,
    233.08188075904496,
    246.94165062806206,
    261.6255653005986,
    277.1826309768721,
    293.6647679174076,
    311.1269837220809,
    329.6275569128699,
    349.2282314330039,
    369.9944227116344,
    391.99543598174927,
    415.3046975799451,
    440,
    466.1637615180899,
    493.8833012561241,
    523.2511306011972,
    554.3652619537442,
    587.3295358348151,
    622.2539674441618,
    659.2551138257398,
    698.4564628660078,
    739.9888454232688,
    783.9908719634985,
    830.6093951598903,
    880,
    932.3275230361799,
    987.7666025122483,
    1046.5022612023945,
    1108.7305239074883,
    1174.6590716696303,
    1244.5079348883237,
    1318.5102276514797,
    1396.9129257320155,
    1479.9776908465376,
    1567.981743926997,
    1661.2187903197805,
    1760,
    1864.6550460723597,
    1975.533205024496,
    2093.004522404789,
    2217.4610478149766,
    2349.31814333926,
    2489.0158697766,
    2637.02045530296,
    2793.825851464031,
    2959.955381693075,
    3135.9634878539946,
    3322.437580639561,
    3520,
    3729.3100921447194,
    3951.066410048992,
    4186.009044809578,
    4434.922095629953,
    4698.63628667852,
    4978.031739553295,
    5274.04091060592,
    5587.651702928062,
    5919.91076338615,
    6271.926975707989,
    6644.875161279122,
    7040,
    7458.620184289437,
    7902.132820097988,
    8372.018089619156,
    8869.844191259906,
    9397.272573357044,
    9956.06347910659,
    10548.081821211836,
    11175.303405856126,
    11839.8215267723,
    12543.853951415975,
];

let allowAudioButton = document.querySelector("#allowAudioButton");
let reverbButton = document.querySelector("#reverbButton");
let filterbButton = document.querySelector("#filterbButton");
let reverbButtonOutput = document.querySelector("#reverbButtonOutput");
let filterButtonOutput = document.querySelector("#filterButtonOutput");
let buttons = document.getElementsByClassName("buttonOsz");
let sliders = document.getElementsByClassName("form-range");
let reverbList = document.querySelector("#btn-group-reverb");
let filterList = document.querySelector("#btn-group-filter");
let reverbOutput = document.getElementById("reverbOutput");
let filterOutput = document.getElementById("filterOutput");
let isAudioEnabled = true;
let ButtonGroupEnabled = true;
let cameraEnabled = false;

let context = new AudioContext();
let masterGain = context.createGain();

let convolver = context.createConvolver();
let filter = context.createBiquadFilter();
let distortion = context.createWaveShaper();

let lfo = context.createOscillator();
let lfoGain = context.createGain();

let oscillators = [[], []];
let velocityVolumes = [[], []];

let attack = 0.1;
let release = 0.1;
let octaveShifter = 60;

lfoGain.gain.value = 0.05;
lfo.frequency.value = 6;
lfo.start(context.currentTime);
lfo.connect(lfoGain);

masterGain.gain.value = 1;

distortion.connect(filter);
filter.connect(convolver);
convolver.connect(masterGain);
masterGain.connect(context.destination);

distortion.curve = makeDistortionCurve(0);
distortion.oversample = "4x";

let cameraButton = document.querySelector("#cameraButton");
let mediaContainer = document.querySelector("#mediaContainer");

let displayMediaOptions = {
    video: { width: 400, height: 400 },
    audio: false,
};

cameraButton.addEventListener("click", function (e) {
    cameraEnabled = !cameraEnabled;

    if (cameraEnabled) {
        console.log("cameraButton: true");
        mediaContainer.classList.add("embed-responsive-16by9");
        cameraButton.classList.remove("btn-danger");
        cameraButton.classList.add("btn-success");
        mediaContainer.innerHTML = "<video autoplay='true' id='videoElement'></video>";
        if (navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getDisplayMedia()
                .then(function (stream) {
                    document.querySelector("#videoElement").srcObject = stream;
                })
                .catch(function (err0r) {
                    console.log("Something went wrong!");
                    mediaContainer.classList.remove("embed-responsive-16by9");
                    cameraButton.classList.remove("btn-success");
                    cameraButton.classList.add("btn-danger");
                    console.log("cameraButton: false")
                    mediaContainer.innerHTML = "<img class='img-fluid shadow-2-strong' src='img/noSignal.jpg' />";
                });
        }
    } else {
        mediaContainer.classList.remove("embed-responsive-16by9");
        cameraButton.classList.remove("btn-success");
        cameraButton.classList.add("btn-danger");
        console.log("cameraButton: false")
        mediaContainer.innerHTML = "<img class='img-fluid shadow-2-strong' src='img/noSignal.jpg' />";
    }
});

for (let i = 0; i < sliders.length; i++) {
    sliders[i].addEventListener("mousemove", changeParameter);
}

function changeParameter() {
    switch (this.id) {
        case "attackSlider":
            attack = this.value / 100;
            document.querySelector("#attackOutput").innerHTML = attack + " sec";
            break;
        case "releaseSlider":
            release = this.value / 100;
            document.querySelector("#releaseOutput").innerHTML = release + " sec";
            break;
        case "frequencySlider":
            filter.frequency.value = this.value;
            document.querySelector("#frequencyOutput").innerHTML = this.value + " Hz";
            break;
        case "detuneSlider":
            filter.detune.value = this.value;
            document.querySelector("#detuneOutput").innerHTML = this.value + " cents";
            break;
        case "qSlider":
            filter.Q.value = this.value;
            document.querySelector("#qOutput").innerHTML = this.value + " ";
            break;
        case "gainSlider":
            filter.gain.value = this.value;
            document.querySelector("#gainOutput").innerHTML = this.value + " dB";
            break;
        case "distortionSlider":
            distortion.curve = makeDistortionCurve(this.value);
            document.querySelector("#distortionOutput").innerHTML = this.value;
            break;
        case "lfoSlider":
            lfo.frequency.value = this.value;
            document.querySelector("#lfoOutput").innerHTML = this.value + "Hz";
            break;
        case "masterGainSlider":
            masterGain.gain.value = this.value / 100;
            document.querySelector("#masterGainOutput").innerHTML = this.value / 100;
            break;
    }
}

//Audiograph figure it out... das ende kommt in den masterGain

allowAudioButton.addEventListener("click", function (e) {
    allowAudioButton.innerHTML = "Audio allowed";
    allowAudioButton.classList.remove("btn-danger");
    allowAudioButton.classList.add("disabled");
    allowAudioButton.classList.add("btn-success");
    console.log("Audio Allowed");
    filter.connect(masterGain);
});

// ButtonGroups
let reverbButtonList = ["room", "garage", "church", "cave"];
let filterButtonList = ["lowpass", "highpass", "bandpass", "allpass", "lowshelf", "Highshelf", "peaking", "notch"];

filterList.addEventListener("mousedown", function (e) {
    let name = e.target.value;
    if (name != null) {
        changeActiveButtonClass(name, filterButtonList);
        filter.type = name;
    }

});

filter.type = "lowpass";

reverbList.addEventListener("mousedown", function (e) {
    let name = e.target.value;
    if (name != null) {
        changeActiveButtonClass(name, reverbButtonList);
        loadImpulseResponse(name);
    }
});

function changeActiveButtonClass(name, buttonList) {
    buttonList = buttonList;
    element = document.getElementById(name);

    for (r in buttonList) {
        document.getElementById(buttonList[r]).classList.remove("active");
    }

    if (element != null) {
        element.classList.add("active");
    }

};

function loadImpulseResponse(selectedReverb) {
    fetch("impulseResponses/" + selectedReverb + ".wav")
        .then((response) => response.arrayBuffer())
        .then((undecodedAudio) => context.decodeAudioData(undecodedAudio))
        .then((audioBuffer) => {
            if (convolver) {
                convolver.disconnect();
            }

            convolver = context.createConvolver();
            convolver.buffer = audioBuffer;
            convolver.normalize = true;

            if (ButtonGroupEnabled) {
                filter.disconnect();
                filter.connect(convolver);
                convolver.connect(masterGain);
            } else {
                distortion.connect(convolver);
                convolver.connect(masterGain);
            }

        })
        .catch(console.error);
}

for (let i = 0; i < 127; i++) {
    for (let j = 0; j < 2; j++) {
        velocityVolumes[j][i] = context.createGain();
        velocityVolumes[j][i].connect(distortion);
    }
}

let octaveShifterButtonUp = document.querySelector("#octaveShifterButtonUp");
let octaveShifterButtonDown = document.querySelector("#octaveShifterButtonDown");
let octaveShifterTextOutput = document.querySelector("#octaveShifterTextOutput");
let octaveShifterCounter = 0;

octaveShifterFunction(octaveShifterButtonUp, octaveShifterButtonDown, octaveShifterTextOutput);

function octaveShifterFunction(ButtonUp, ButtonDown, TextOutput) {
    ButtonUp.addEventListener("click", function () {
        octaveShifterCounter += 1;
        TextOutput.innerHTML = octaveShifterCounter;
        console.log(octaveShifterCounter);
    });

    ButtonDown.addEventListener("click", function () {
        octaveShifterCounter -= 1;
        TextOutput.innerHTML = octaveShifterCounter;
        console.log(octaveShifterCounter);
    });
}

function startNote(note, velocity) {
    let oscTypes = ["sine", "sawtooth"];

    note += octaveShifterCounter * 14;

    for (let i = 0; i < 2; i++) {

        oscillators[i][note] = context.createOscillator();
        oscillators[i][note].type = oscTypes[i];
        oscillators[i][note].frequency.value = allFrequencies[note];

        if (i === 1) {
            oscillators[i][note].detune.value = 2;
        }

        velocityVolumes[i][note].gain.cancelScheduledValues(0);
        velocityVolumes[i][note].gain.setValueAtTime(0, context.currentTime);

        lfoGain.connect(velocityVolumes[i][note].gain);
        oscillators[i][note].connect(velocityVolumes[i][note]);
        oscillators[i][note].start(context.currentTime);

        velocityVolumes[i][note].gain.linearRampToValueAtTime(0.05 + velocity / 127, context.currentTime + attack);
    }
}

function stopNote(note, velocity) {

    note += octaveShifterCounter * 14;

    for (let i = 0; i < 2; i++) {
        velocityVolumes[i][note].gain.cancelScheduledValues(0);
        velocityVolumes[i][note].gain.linearRampToValueAtTime(0, context.currentTime + attack + release);
        oscillators[i][note].stop(context.currentTime + attack + release + 0.1);
    }
}

function controlChange(controllerNr, value) {
    // do something...
}

function pitchBend(LSB, HSB) {
    // do something...
}

function makeDistortionCurve(amount) {
    let n_samples = 44100,
        curve = new Float32Array(n_samples);

    for (var i = 0; i < n_samples; ++i) {
        var x = (i * 2) / n_samples - 1;
        curve[i] = ((Math.PI + amount) * x) / (Math.PI + amount * Math.abs(x));
    }

    return curve;
}

buttonGroupSwitch(reverbButton, reverbButtonList, reverbButtonOutput);
buttonGroupSwitch(filterButton, filterButtonList, filterButtonOutput);

function buttonGroupSwitch(button, buttonList, textOutput) {
    button.addEventListener("click", function (e) {
        if (ButtonGroupEnabled) {
            // sound.pause();
            textOutput.innerHTML = "Enabled";
            console.log("On");
            for (r in buttonList) {
                document.getElementById(buttonList[r]).classList.remove("disabled");
            }

        } else {
            // sound.play();
            textOutput.innerHTML = "Disabled";
            console.log("Off");

            filter.disconnect();
            filter.connect(masterGain);

            for (r in buttonList) {
                document.getElementById(buttonList[r]).classList.add("disabled");
            }
        }

        ButtonGroupEnabled = !ButtonGroupEnabled;
    });
}
